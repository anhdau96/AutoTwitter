...
public class TweetLinkController {
    public List<LinkPin> getLinkTweet(){
        ...
        try {
            ...
            PreparedStatement pst = connect.prepareStatement("SELECT * FROM links WHERE check_tweet = 0 LIMIT 200");
    ...
    public void updateLinkTweet(int idlink){
        ...
        try {
            Connection connect = connectDB.getConnectLink();
            PreparedStatement pst = connect.prepareStatement("UPDATE links SET check_tweet = 1 WHERE id = ?");
            pst.setInt(1, idlink);
            pst.execute();
            connectDB.closeConnectLink();
        } catch (ClassNotFoundException | SQLException ex) {
